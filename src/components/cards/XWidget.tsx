...
const XWidget = () => {
  ...
  const currentTheme = themeStyles[xTheme as keyof typeof themeStyles] || themeStyles.dark;

  interface VerifiedBadgeProps {
    type: 'none' | 'blue' | 'gold' | 'grey';
  }

  const VerifiedBadge = ({ type }: VerifiedBadgeProps) => {
    ...
    return (
      <BadgeCheck
        className={`w-[18px] h-[18px] fill-current ${colors[type as keyof typeof colors]} inline-block ml-1 mb-0.5`}
      />
    );
  };
