...
const LyricsWidget = ({
  ...
  const [displayLines, setDisplayLines] = useState<number[]>(selectedLines);
  ...
  const animatedIndices = useRef<Set<number>>(new Set(selectedLines));
  ...
  useEffect(() => {
    if (!gsap || !containerRef.current || !widgetRef.current) return;

    const el = widgetRef.current;

    // Measure natural height immediately
    gsap.set(el, { height: "auto", width: "" });
    ...
    if (
      ...
      gsap.fromTo(
        ...
        { height: lastState.current.height, width: lastState.current.width },
        {
          ...
          ease: "expo.out",
          overwrite: true,
          onComplete: () => gsap.set(el, { clearProps: "height,width" }),
        },
      );
    } else {
      gsap.set(el, { clearProps: "height,width" });
    }
  ...
  // 3. Auto-scroll logic for Video/Preview
  useEffect(() => {
    if (currentTime !== undefined && containerRef.current) {
      const activeLine = containerRef.current.querySelector(
        `[data-line-idx="${displayLines[0]}"]`,
      );
      if (activeLine) {
        gsap.to(containerRef.current, {
          y:
            -(activeLine as HTMLElement).offsetTop +
    ...
    >
        ...
        {hasText ? (
          displayLines.map((idx, i) => (
            <p
              key={idx}
              data-line-idx={idx}
              className="text-zinc-900 dark:text-white text-[5rem] font-bold tracking-tight"
            >
              {lyrics[idx]}
            </p>
          ))
        ) : (
          <p className="text-zinc-400/50 dark:text-white/20 text-5xl font-bold italic">
            Select lyrics...
          </p>
        )}
      </div>
      ...
      <div className="flex items-center gap-8 mt-6">
        {coverImage ? (
          ...
          <div className="w-28 h-28 rounded-[2rem] bg-zinc-100 dark:bg-zinc-800" />
        )}
        <div className="flex flex-col flex-1 text-left min-w-0">
          <span className="text-zinc-900 dark:text-white text-[2.75rem] font-bold line-clamp-1 leading-tight">
            {track?.title || "Track Title"}
          </span>
          <span className="text-zinc-500 dark:text-zinc-400 text-3xl line-clamp-1 mt-2">
            {track?.artist || "Artist Name"}
          </span>
        </div>
      </div>
