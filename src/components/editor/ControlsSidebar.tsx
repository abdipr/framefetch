        ...
        <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          {preset !== "lockscreen" && (
            <section className="flex flex-col gap-4">
              ...
              <Tabs
                value={mode}
                onValueChange={(val) => setMode(val as "lyrics" | "player")}
                className="w-full"
              >
            ...
            <div className="flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-6 bg-white dark:bg-zinc-900/50 p-5 rounded-[2.5rem] border border-zinc-200 dark:border-zinc-800 shadow-sm">
                {preset !== "lockscreen" && (
                  <>
                    <div
                      className={`flex flex-col gap-3 ${isCardOnly ? "opacity-40 pointer-events-none" : ""}`}
                    >
                      <div className="flex justify-between items-center px-1">
                    ...
                    <div className="flex items-center justify-between px-1 pt-2">
                      ...
                      >
                        <div
                          className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all duration-300 ${isCardOnly ? "left-5 shadow-sm" : "left-1"}`}
                        />
                      </button>
                    </div>
                  </>
                ...
                <div className="flex flex-col gap-6 pt-4 border-zinc-100 dark:border-zinc-800">
              ...
              {preset !== "lockscreen" && (
                <div
                  className={`flex flex-col gap-4 ${isCardOnly ? "opacity-40 pointer-events-none" : ""}`}
                >
                  <label className="text-[10px] font-black text-zinc-400 uppercase tracking-[0.2em] px-1">
                    Aspect Ratio
                  </label>
                  <div className="grid grid-cols-3 gap-3">
                    {["1:1", "3:4", "9:16"].map((f) => (
                      <button
                        key={f}
                        disabled={isCardOnly}
                        onClick={() => setFormat(f as "1:1" | "3:4" | "9:16")}
            ...
            {SHOW_VIDEO_FEATURES && (
              <div className="flex flex-col gap-6 pt-8 mt-4 border-t border-zinc-200 dark:border-zinc-800">
                ...
                {!syncedLyrics || syncedLyrics.length === 0 ? (
                  <div className="bg-zinc-50 dark:bg-zinc-900/50 p-6 rounded-[2rem] border border-zinc-200 dark:border-zinc-800 border-dashed text-center">
                    <p className="text-[11px] text-zinc-400 font-medium leading-relaxed">
                      Synced lyrics not available for this track.
                      <br />
                      Video export requires timestamps.
                    </p>
                  </div>
                ) : (
                  <div className="flex flex-col gap-5 bg-zinc-50 dark:bg-zinc-900/50 p-5 rounded-[2.5rem] border border-zinc-200 dark:border-zinc-800">
                    <div className="grid grid-cols-2 gap-4">
                      <div className="flex flex-col gap-2">
                        ...
                        <Select
                          value={videoRange[0].toString()}
                          onValueChange={(val) => {
                            if (!val) return;
                            const v = parseInt(val);
                            setVideoRange([v, Math.max(v, videoRange[1])]);
                          }}
                        >
                          <SelectTrigger className="w-full h-10 px-3 rounded-xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-xs font-bold">
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        <Select
                          value={videoRange[1].toString()}
                          onValueChange={(val) => {
                            if (!val) return;
                            const v = parseInt(val);
                            setVideoRange([
                              videoRange[0],
                              Math.max(videoRange[0], v),
                            ]);
                          }}
                        >
                          <SelectTrigger className="w-full h-10 px-3 rounded-xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-xs font-bold">
                    ...
                    <div className="grid grid-cols-2 gap-4 pt-2">
                      <div className="flex flex-col gap-2">
                        ...
                        <Select
                          ...
                          onValueChange={(val) =>
                            setVideoSettings({ resolution: val as any })
                          }
                        >
                          <SelectTrigger className="w-full h-10 px-3 rounded-xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-xs font-bold">
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        <Select
                          ...
                          onValueChange={(val) => {
                            if (!val) return;
                            setVideoSettings({ fps: parseInt(val) });
                          }}
                        >
                          <SelectTrigger className="w-full h-10 px-3 rounded-xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-xs font-bold">
                    ...
                    <div className="flex items-center justify-between px-1 py-1 pt-2">
                      <div className="flex flex-col gap-0.5">
                        ...
                        <span className="text-[11px] font-bold text-zinc-500">
                          {(() => {
                            const start =
                              syncedLyrics[videoRange[0]]?.time || 0;
                            const end = syncedLyrics[videoRange[1]]?.time || 0;
                            const dur = Math.max(0, end - start + 3);
                      ...
                      <button
                        ...
                        className="bg-zinc-900 dark:bg-white text-white dark:text-black px-6 py-2.5 rounded-full text-[11px] font-black uppercase tracking-wider hover:scale-105 active:scale-95 transition-all shadow-lg"
                      >
                        Render MP4
                      </button>
                    </div>
              ...
              <div className="grid grid-cols-1 gap-5 bg-zinc-50 dark:bg-zinc-900/50 p-5 rounded-[2.5rem] border border-zinc-200 dark:border-zinc-800">
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <Tabs
                    value={exportFormat}
                    onValueChange={(val) =>
                      setExportSettings({ exportFormat: val as "png" | "jpg" })
                    }
                    className="w-full"
                  >
